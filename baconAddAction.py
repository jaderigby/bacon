...
What would you like to call the action? ''')
	...
	basicSnippet = '''import baconMessages as msg
...
def execute():
	msg.example()
'''
	argSnippet = '''import bacoMessages as msg
import helpers
...
LOCATION:       {}'''.format(name, newFeature, newAction, finalPath)
	...
	data = helpers.read_file(fullPath + '/baconActions.py')
...
# new actions start here'''.format(newFeature=newFeature, newAction=newAction)
	...
	helpers.write_file(fullPath + '/baconActions.py', data)
	actionData = json.loads(helpers.read_file(fullPath + '/baconAction-list.json'))
	newItem = {}
	newItem['name'] = newAction
	newItem['description'] = ''
	actionData['actions'].append(newItem)
	helpers.write_file(fullPath + '/baconAction-list.json', json.dumps(actionData, indent=4))
	msg.done()
